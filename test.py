...
"""
This Python script is designed to run conformal inference on a dataset using 
various combinations of variables and configurations. The script processes input 
files from a specified directory, applies different variable combinations for 
conformal inference, and outputs the results to another directory. The script is 
configured via command-line arguments, making it flexible for different datasets 
and settings.
...
Usage:
    To execute the script, the user needs to provide the necessary command-line 
    arguments. It's designed to be run in a terminal or command-line interface 
    where Python and the required libraries (numpy, pandas, torch, etc.) are installed.
    
"""
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Description of your program")
    parser.add_argument(
        "-i", "--input_dir", default="./input_real", help="Input_directory"
    )
    parser.add_argument(
        "-o", "--output_dir", default="./output", help="Output_directory"
    )
    ...
    for f in onlyfiles:
        ...
        for combination in range(len(all_combinations)):
            ...
            for alpha in alphas:
                ...
                df2["filename"] = np.repeat(
                    os.path.basename(f), repeats=len(ci_results["condition_list"])
                )
                df2["combination"] = np.repeat(
                    combination, repeats=len(ci_results["condition_list"])
                )
                df2["alpha"] = np.repeat(
                    alpha, repeats=len(ci_results["condition_list"])
                )
